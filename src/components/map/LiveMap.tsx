...
function DriverMarkerWithStream({
  driverId,
  driverName,
  initialLocation,
}: {
  ...
  // Subscribe to real-time SSE telemetry for this driver
  const { location, isConnected } = useDriverStream(driverId);

  // Use live SSE coordinates if available, otherwise fall back to initial DB location
      ...
      <Popup>
        <div className="font-sans">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            {isConnected && (
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" title="Live SSE Active" />
            )}
          </div>
          <p className="text-xs text-zinc-500">
            {isConnected ? "⚡ Live Telemetry Active" : "Active Unit"}
          </p>
        </div>
      </Popup>
    </Marker>
        ...
        {routes.map((route) => {
          ...
          return (
            <React.Fragment key={routeLayerKey}>
                  ...
                  <Popup>
                    <div className="p-1">
                      ...
                      <div className="flex items-center justify-between gap-4 mt-2 pt-2 border-t border-zinc-100">
                        ...
                        <span className={`text-[10px] tracking-wider font-bold px-2 py-0.5 rounded-sm ${delivery.status === 'DELIVERED' ? 'bg-emerald-100 text-emerald-700' :
              ...
              <DriverMarkerWithStream
                key={route.driverId}
                driverId={route.driverId}
                driverName={route.driverName}
                initialLocation={route.driverLocation}
              />
            </React.Fragment>
          );
        })}
